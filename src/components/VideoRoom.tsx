  ...
  useEffect(() => {
    ...
    const initConnections = () => {
      ...
      newPeer.on('open', (id) => {
        ...
        console.log('[RTC-CHECK] [2] Sending join event to socket with details:', joinPayload);
        newSocket?.emit('join', joinPayload);
      ...
      newSocket.on('match-found', (data: { uid: string; displayName: string; photoURL: string; peerId: string; remoteSocketId: string; remoteCountry: string; remoteCountryCode: string }) => {
        ...
        if (isCaller) {
          console.log('[RTC-CHECK] [7] Caller initiating PeerJS outgoing call to Remote PeerID:', data.peerId, 'LocalStream tracks:', localStream?.getTracks().map(t => t.kind));
