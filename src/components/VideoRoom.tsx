      ...
      try {
        ...
        if (err instanceof Error) {
          if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
            setCamError('Camera Permission Denied: Please click the camera icon in your browser address bar and select "Allow" to use OMIX.');
  ...
  useEffect(() => {
    ...
    const initConnections = () => {
      ...
      newSocket.on('match-found', (data: { uid: string; displayName: string; photoURL: string; peerId: string; remoteSocketId: string; remoteCountry: string; remoteCountryCode: string }) => {
        ...
        console.log('[DEBUG] Calling remote peer now:', data.peerId);
      ...
      newSocket.on('peer-disconnected', () => {
        console.log('[DEBUG] Partner disconnected, triggering findNextMatch');
        findNextMatch();
      });
    };
        ...
        <div className="relative z-10 max-w-md w-full space-y-8 p-8 rounded-[2.5rem] bg-slate-900/40 border border-white/5 backdrop-blur-3xl shadow-2xl">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              {camError 
                ? camError 
                : "OMIX is a live video matching platform. Please allow camera access when prompted by your browser so others can see you."}
            </p>
          </div>
