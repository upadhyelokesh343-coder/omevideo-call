...
export function VideoRoom({ isVIP, matchingPreference, userGender, targetCountry, userCountry, countryCode, initialStream, uid, displayName, photoURL, onUpgrade, onExit, socketProp }: VideoRoomProps) {
  ...
  useEffect(() => {
    // Only initialize Peer and Socket connections when camera/mic are ready.
    ...
    const initConnections = () => {
      ...
      if (socketProp) {
        ...
        newSocket.removeAllListeners(); // Important: prevent duplicate listeners if component remounts
        console.log('[DEBUG] Using socket provided from props');
      } else {
        newSocket = io({
