...
interface VideoRoomProps {
  ...
  onUpgrade: () => void;
  onExit: () => void;
  socketProp?: Socket | null;
}

export function VideoRoom({ isVIP, matchingPreference, userGender, targetCountry, userCountry, countryCode, initialStream, uid, displayName, photoURL, onUpgrade, onExit, socketProp }: VideoRoomProps) {
  ...
  // Strict VIP Enforcement: 
  // - Girls-Only filter requires isGirlsVip
  // - Boys/Everyone filter requires isEveryoneVip
  const isVipRequired = 
    (matchingPreference === 'female' && !profile?.isGirlsVip) ||
    (matchingPreference === 'male' && !profile?.isEveryoneVip);
  ...
  useEffect(() => {
    if (initialStream.getTracks().length > 0) {
  ...
  useEffect(() => {
    return () => {
      console.log('[DEBUG] VideoRoom Unmounting: Final Teardown');
      if (socketRef.current && !socketProp) {
  ...
  useEffect(() => {
    // Only initialize Peer and Socket connections when media stream is ready.
    if (isCamRequesting || camError || !localStream || localStream.getTracks().length === 0) {
      console.log('[DEBUG] Connection init waiting: ', { isCamRequesting, hasCamError: !!camError, hasStream: !!localStream });
      return;
    }
    ...
    const initConnections = () => {
      ...
      // If we already have a socket/peer and it's the right one, don't re-init
      if (socketRef.current && peerRef.current) {
        if (socketProp && socketRef.current === socketProp) return;
        if (!socketProp) return;
      }
      
      // Cleanup old ones if we are re-initializing
      if (socketRef.current && !socketProp) socketRef.current.disconnect();
      if (peerRef.current) peerRef.current.destroy();

      if (socketProp) {
        newSocket = socketProp;
        newSocket.removeAllListeners(); // Prevent duplicate listeners
        console.log('[DEBUG] Using socket provided from props. Socket ID:', newSocket.id);
      } else {
        newSocket = io({
      ...
      console.log('[DEBUG] Creating local PeerJS Client with options:', {
        host: peerOptions.host,
        path: peerOptions.path,
        port: peerOptions.port,
        secure: peerOptions.secure,
        iceServers: peerOptions.config.iceServers.map((s: any) => s.urls)
      });

      // Local PeerJS Server connection configuration with enterprise STUN/TURN backup
      ...
      newSocket.on('connect', () => {
        console.log('[RTC-CHECK] [1] Socket.IO connection established. Socket ID:', newSocket?.id);
      });

      newSocket.on('connect_error', (err) => {
        console.error('[RTC-CHECK] [1] Socket.IO connection error:', err);
      });

      newSocket.on('disconnect', (reason) => {
        console.log('[RTC-CHECK] [1] Socket.IO disconnected. Reason:', reason);
      });

      newPeer.on('open', (id) => {
        console.log('[RTC-CHECK] [6] PeerJS local signaling "open" fired. Assigned PeerID:', id);
        setPeerId(id);
      });

      newPeer.on('close', () => {
        console.log('[DEBUG] PeerJS connection closed');
        setPeerId(null);
      });

      newPeer.on('disconnected', () => {
        console.log('[DEBUG] PeerJS signaling disconnected, attempting reconnection...');
        newPeer?.reconnect();
      });
      ...
      const setupPeerConnectionDebug = (pc: RTCPeerConnection) => {
        ...
        pc.addEventListener('connectionstatechange', () => {
          console.log('[RTC-CHECK] [12] Native RTCPeerConnection connectionState changed:', pc.connectionState);
          if (pc.connectionState === 'connected') {
            console.log('[RTC-CHECK] [12] RTCPeerConnection reached connected state!');
        ...
        pc.addEventListener('iceconnectionstatechange', () => {
          ...
          if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
            console.log('[RTC-CHECK] [12] ICE connection state reached connected/completed!');
            setIsConnectionEstablished(true);
        ...
        pc.addEventListener('icegatheringstatechange', () => {
          console.log('[RTC-CHECK] [10] ICE Gathering State changed:', pc.iceGatheringState);
        });
        pc.addEventListener('signalingstatechange', () => {
          console.log('[RTC-CHECK] [9] signalingState changed:', pc.signalingState);
          if (pc.signalingState === 'have-local-offer') {
            console.log('[RTC-CHECK] [9] WebRTC Offer generated and set locally.');
          } else if (pc.signalingState === 'have-remote-offer') {
            console.log('[RTC-CHECK] [9] WebRTC Offer received from remote and set.');
          } else if (pc.signalingState === 'stable' && (pc.connectionState === 'new' || pc.connectionState === 'connecting')) {
            console.log('[RTC-CHECK] [9] WebRTC Answer exchanged, signaling stable.');
          }
        });
        ...
        pc.onicecandidate = (event) => {
          if (event.candidate) {
            console.log('[RTC-CHECK] [10] Local ICE candidate generated:', event.candidate.candidate);
            if (event.candidate.candidate.includes('relay')) {
              console.log('[RTC-CHECK] [11] TURN server successfully reached! Relay candidate generated:', event.candidate.candidate);
            }
          } else {
            console.log('[RTC-CHECK] [10] Local ICE candidate generation finished (null candidate received)');
          }
        ...
        pc.ontrack = (event) => {
          console.log('[RTC-CHECK] [13] ontrack event fired! Received remote media track kind:', event.track.kind);
          if (origOnTrack) origOnTrack.call(pc, event);
        };
      };

      newPeer.on('call', (call) => {
        console.log('[RTC-CHECK] [8] Receiver received "call" event. Incoming call from Remote PeerID:', call.peer);
        ...
        console.log('[DEBUG] Receiver answering incoming call with localStream...');
        call.answer(localStreamRef.current);
        
        call.on('stream', (remoteStream) => {
          console.log('[RTC-CHECK] [13] Remote stream received on incoming call. Tracks:', remoteStream.getTracks().map(t => t.kind));
          setIsConnectionEstablished(true);
          console.log('[RTC-CHECK] [14] Binding remote stream to HTMLVideoElement.');
      ...
      newSocket.on('match-found', (data: { uid: string; displayName: string; photoURL: string; peerId: string; remoteSocketId: string; remoteCountry: string; remoteCountryCode: string }) => {
        console.log('[RTC-CHECK] [MATCH-FOUND] Received match-found event!', data);
        if (matchingTimerRef.current) {
          clearTimeout(matchingTimerRef.current);
          matchingTimerRef.current = null;
        }
        ...
        // Anti-Glare Caller/Receiver Logic
        const currentSocketId = newSocket?.id || '';
        const isCaller = currentSocketId ? (currentSocketId < data.remoteSocketId) : false;

        if (isCaller) {
          console.log('[RTC-CHECK] [CALLER] Initiating PeerJS call to:', data.peerId);
          const call = newPeer?.call(data.peerId, localStreamRef.current);
          activeCallRef.current = call;
          ...
          if (call) {
            ...
            call.on('stream', (remoteStream) => {
              console.log('[RTC-CHECK] [STREAM] Remote stream received (Caller side)');
              setIsConnectionEstablished(true);
              playVideoSafely(remoteVideoRef.current, remoteStream, isRemoteVideoLoadingRef);
            });

            call.on('close', () => {
              console.log('[RTC-CHECK] [CLOSE] Call closed (Caller side)');
              setIsConnectionEstablished(false);
            });

            call.on('error', (err) => {
              console.error('[RTC-CHECK] [ERROR] Call error (Caller side):', err);
              setIsConnectionEstablished(false);
            });
          }
        } else {
          console.log('[RTC-CHECK] [RECEIVER] Waiting for incoming call from:', data.peerId);
        }
      });
  ...
  }, [socketProp, isCamRequesting, camError, !!localStream]);

  // Separate effect for joining and settings updates
  useEffect(() => {
    if (!socket || !peerId) return;

    const joinPayload = {
      isVIP: profile?.isVip || profile?.isGirlsVip || profile?.isEveryoneVip || false,
      isGirlsVip: profile?.isGirlsVip || false,
      isEveryoneVip: profile?.isEveryoneVip || false,
    ...
    console.log('[RTC-CHECK] [JOIN] Emitting join and find-match with settings:', {
      matchingPreference,
      targetCountry,
      isVIP: joinPayload.isVIP
    });
  ...
  }, [socket, peerId, matchingPreference, targetCountry, isVIP, profile?.isGirlsVip, profile?.isEveryoneVip, profile?.isVip]);

  // --- 3. CONTINUOUS SCAN ENGINE REMOVED ---

  const findNextMatch = () => {
    console.log('[RTC-CHECK] [REQUEUE] findNextMatch called. Clearing state and requesting new partner.');
    if (matchingTimerRef.current) clearTimeout(matchingTimerRef.current);
  ...
  if (isVipRequired) {
    return (
      <div className="relative flex h-[100dvh] w-full flex-col bg-slate-950 items-center justify-center p-6 text-center text-white select-none">
        <div className="absolute inset-0 bg-gold-600/5 blur-[120px] rounded-full pointer-events-none" />
        
        <div className="relative z-10 max-w-md w-full space-y-8 p-8 rounded-[2.5rem] bg-slate-900/40 border border-gold-500/20 backdrop-blur-3xl shadow-2xl">
          <div className="h-20 w-20 mx-auto rounded-3xl bg-gold-500/10 flex items-center justify-center border border-gold-500/20 text-gold-500">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              Matchmaking by {matchingPreference === 'female' ? 'Girls' : 'Everyone (Strictly Male)'} is a premium feature. Upgrade to Omio VIP to unlock this filter and enjoy exclusive benefits.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <button
              onClick={onUpgrade}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-gold-600 to-amber-600 font-bold hover:opacity-90 active:scale-95 transition-all text-white shadow-xl shadow-gold-500/20"
            >
              Get VIP Now
            </button>
            <button
              ...
              >
                {showFallback ? (
                  <div className="text-center p-8">
                     ...
                     <h3 className="text-white font-bold text-xl mb-2">No users found in {targetCountry}</h3>
                  ...
                  <>
                    ...
                    <h2 className="text-xl font-bold text-white mb-1">
                      {remotePeerId ? "Connecting Video..." : peerId ? "Finding a Match" : "Connecting to Server..."}
                    </h2>
        ...
        <div className="relative flex-1 w-full lg:w-1/2 bg-slate-950 flex items-center justify-center overflow-hidden aspect-[9/16] lg:aspect-auto">
          ...
          <div className="absolute bottom-4 left-4 z-30 flex flex-col items-start gap-2 pointer-events-none">
            ...
            <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-2 py-1 rounded-full border border-white/10">
              ...
              />
              <span className="text-[8px] font-bold uppercase tracking-tighter text-white/70">You ({userCountry})</span>
            </div>
          </div>
