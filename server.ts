...
io.on('connection', (socket) => {
  console.log('[RTC-CHECK] [1] Socket.IO client connected. Socket ID:', socket.id);
  broadcastUserCount();
  ...
  socket.on('join', (data: { 
    ...
    console.log(`[RTC-CHECK] [2] User successfully joined matchmaking register. ID: ${socket.id}, Name: ${data.displayName}, PeerID: ${data.peerId}`);
  });
  ...
  socket.on('find-match', () => {
    ...
    // Handle skip: notify previous active partner that this user has disconnected/skipped
    if (currentUser.activePartnerId) {
      ...
      if (prevPartner) {
        ...
        console.log(`[SKIP] Notifying ${prevPartnerId} that partner ${socket.id} has left/skipped`);
    ...
    if (matchId) {
      ...
      if (peer) {
        // Set active partner relationships
        currentUser.activePartnerId = matchId;
        peer.activePartnerId = socket.id;

        console.log(`[MATCHMADE] Successfully matched ${socket.id} with partner ${matchId}`);

        // Inform both users
        console.log(`[RTC-CHECK] [4] Emitting match-found to caller: ${socket.id} (PeerID: ${currentUser.peerId}) and receiver: ${matchId} (PeerID: ${peer.peerId})`);
      ...
      console.log(`[RTC-CHECK] [3] Matchmaking queue update after find-match. Current queue size: ${waitingQueue.length}. Queue IDs: ${JSON.stringify(waitingQueue)}`);
    }
  });
  ...
  socket.on('disconnect', () => {
    console.log('[RTC-CHECK] [1] Socket.IO client disconnected. Socket ID:', socket.id);
    const currentUser = users.get(socket.id);
    if (currentUser && currentUser.activePartnerId) {
      ...
      if (partner) {
        ...
        console.log(`[DISCONNECT] Notifying ${partnerId} that partner ${socket.id} has disconnected`);
        io.to(partnerId).emit('peer-disconnected');
      }
    }
    users.delete(socket.id);
    waitingQueue = waitingQueue.filter(id => id !== socket.id);
    console.log(`[RTC-CHECK] [3] Matchmaking queue update after disconnect. Current queue size: ${waitingQueue.length}. Queue IDs: ${JSON.stringify(waitingQueue)}`);
    broadcastUserCount();
  });
});
